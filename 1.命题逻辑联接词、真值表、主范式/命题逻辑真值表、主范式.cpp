...
int main()
{
	...
	//公式中不存在合法运算符
	if (formula.length() == proposition_set.size())
	{
		cout << "输入的公式不含运算符！" << endl;
	}
	//若输入的运算式没有命题变项时，直接退出程序；
	if (proposition_set.size() == 0)
	{
		cout << "输入的公式不含命题变项！" << endl;
		return 0;
	}
	cout << formula << endl;
	int* m;
...
//合法性判断
//就是将输入的命题公式进行判断，可能直接把符号放在了最前面之类的
// 还有输入的符号对不对
// 有一种检测方法就是判断数量，命题联结词和符号数之间的关系
// 
//